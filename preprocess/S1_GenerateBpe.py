...
class BPE:
    def __init__(self, out_path,size=1500):
        self.size = size
    ...
    def split(self, input: [], tag):
        # input: 句子分词后的结果

        # 1.按字符拆分，并添加终结符<e>，统计词频
        corpus = {}
        bpe_result = {word_end: 0}
        print("载入数据")
        ...
        for word in input:
            ...
            for c in word:
                if c in bpe_result:
                    ...
                    bpe_result[c] = 1
            bpe_result[word_end] += 1

            # 拆分并添加终结符
            word_split = " ".join(word)
            word_split = word_split + " " + word_end
        ...
        while len(bpe_result) < self.size:
            ...
            if len(bpe_result) % 1000 == 0:
                ...
                self.saveToFile(saved_result,tag)

        return bpe_result

    def saveToFile(self, result,tag):
        path = "{}/{}_{}.txt".format(self.out_path,tag, len(result))
...
if __name__ == '__main__':
    ...
    bpe = BPE(out_path="../raw_data",size=6000) # bpe词典大小设置
    eng = bpe.loadData(corpus_de_path)
    result = bpe.split(eng, "en")

    bpe = BPE(out_path="../raw_data",size=6000)
    de = bpe.loadData(corpus_en_path)

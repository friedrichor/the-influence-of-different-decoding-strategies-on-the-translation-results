...
def load_bpe_dic(encoder_bpe_path, decoder_bpe_path, sort_by_length=False):
    encoder_bpe_result = []
    decoder_bpe_result = []
    if encoder_bpe_path is not None:
    ...
    with open(filename, 'r', encoding='utf-8') as f:
        ...
        while line:
            if use_encoder:
                ...
                encoder_sentence = line
                decoder_sentence = line
...
def load_dataset(file_path):
    ...
    input_file.close()
    # 按照长度降序排序
    sorted_pair.sort(key=lambda x: len(x), reverse=True)
    ...
    while index < num_samples:
        ...
        if use_auto_batch_size:
            if index == 0:
                ...
                batch_size = int((default_max_encoder_length + default_max_decoder_length) * _default_batch_size / (
                            enc_max_len + dec_max_len))  # 此处需要根据自己的配置进行调整
            # print("\n{}\n".format(batch_size))
            enc_max_len = 0
            dec_max_len = 0
        else:
...
def data_generator(
    ...
    while index < num_samples:
        ...
        # 预估下max_tokens对应的batch_size
        enc_max_len = len(data_sorted_pair[index].split("\t")[0].strip().split(char_space)) + 2
        dec_max_len = len(data_sorted_pair[index].split("\t")[1].strip().split(char_space)) + 2
        batch_size = max_tokens // (enc_max_len + dec_max_len)



        batch_size = max(batch_size, 1)

        # 最后一个batch,可能剩余句子数量不够
        if index+batch_size>num_samples:
            batch_size = num_samples-index

        #count+=batch_size
        #print("alread read lines:{},current max_len:{},current batch_size:{}".format(count,(enc_max_len + dec_max_len), batch_size))

        # 遍历这个batch，将其数据放入enc_batch_data和dec_batch_data，并确认实际上的enc_max_len和dec_max_len
        for i in range(0, batch_size):
            line = data_sorted_pair[index + i]
            batch_index = line.split("\t")
            s_source = batch_index[0].strip()
            s_target = batch_index[1].strip()
            ...
            s_source = s_source.split(char_space)
            s_target = s_target.split(char_space)
            enc_max_len = max(len(s_source), enc_max_len)
            dec_max_len = max(len(s_target), dec_max_len)
            enc_batch_data.append(s_source)
            dec_batch_data.append(s_target)
        # print("{},{}".format(enc_max_len,dec_max_len))
